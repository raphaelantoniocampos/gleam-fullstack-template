...
function init3() {
  return new Model2(get_route2(), new User(0));
}
function update_route(model, route) {
  return model.withFields({ route });
}
function update_user(model, user) {
  return model.withFields({ user });
}
...
var UserRequestedApiCall = class extends CustomType {
};
var ApiCallResponded = class extends CustomType {
...
function call(model) {
  return post(
    ...
    object2(toList([["id", int2(model.user.id)]])),
    expect_json(
      message_error_decoder(),
      (var0) => {
        return new ApiCallResponded(var0);
      }
    )
  );
}
...
function home_view(model) {
  return main(toList([]), toList([h1(toList([]), toList([text("Home Page")]))]));
}
...
function handle_api_response(model, response, validate_data, apply_update, error_effects) {
  if (response.isOk()) {
    ...
    if ($.isOk()) {
      let return$ = $[0];
      let _pipe = apply_update(model, return$[0]);
      return effects(_pipe, return$[1]);
    } else {
...
function update(model, msg) {
  ...
  } else if (msg instanceof UserRequestedApiCall) {
    let _pipe = model;
    return effect(_pipe, call(model));
  } else {
    let _pipe = model;
    return none2(_pipe);
  }
...
function init4(_) {
  ...
  return effects(
    _pipe,
    toList([init2(on_url_change), get_user()])
  );
}
function main2() {
